...
def soft_donut(df_counts, names_col, values_col, color_map, center_text):
    ...
    fig.add_annotation(text=center_text, x=0.5, y=0.5, showarrow=False, font=dict(size=18, color="#374151"))
    return fig
...
with tab1:
    ...
    heat = (tmp.pivot_table(index="persona_label", columns="theme_label", values="id", aggfunc="count", fill_value=0)
    ...
    if heat.size == 0:
        ...
        fig_hm = px.imshow(
            ...
            text_auto=True, aspect="auto", color_continuous_scale="Blues", origin="upper"
        )
        fig_hm.update_layout(
            template="simple_white", font=dict(size=17),
            margin=dict(l=10, r=10, t=10, b=10),
            xaxis_title="Theme", yaxis_title="Profession",
